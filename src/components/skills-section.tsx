...
const skillCategories = [
  {
    ...
    skills: [
      { name: "Discord Moderation", level: 98, icon: MessageSquare },
      { name: "Event Hosting", level: 95, icon: Calendar },
      { name: "Ambassador Programs", level: 92, icon: Users },
  ...
  },
  {
    ...
    skills: [
      { name: "Go-to-Market (GTM)", level: 95, icon: Target },
      { name: "User Acquisition", level: 92, icon: TrendingUp },
      { name: "Retention Strategies", level: 90, icon: BarChart3 },
      { name: "Tokenomics Design", level: 85, icon: Coins },
    ],
  },
  {
    ...
    skills: [
      ...
      { name: "Streaming", level: 90, icon: Mic },
      { name: "Branding", level: 85, icon: Palette },
      { name: "Campaign Management", level: 92, icon: PenTool },
    ],
  },
  {
    ...
    skills: [
      { name: "Community Strategy", level: 96, icon: Brain },
      { name: "Partnership Development", level: 88, icon: HandshakeIcon },
      { name: "Web3 Gaming", level: 90, icon: Gamepad2 },
      { name: "Global Community Scaling", level: 92, icon: Globe },
    ],
  },
  {
    ...
    skills: [
      { name: "Zammad Ticketing", level: 90, icon: Settings },
      { name: "AI Tools", level: 85, icon: Bot },
      { name: "Analytics & Metrics", level: 88, icon: BarChart3 },
      { name: "Automation Systems", level: 82, icon: Zap },
    ],
  },
];

const languages = [
  { name: "English", level: 100, flag: "🇺🇸", country: "Fluent" },
  { name: "Urdu", level: 100, flag: "🇵🇰", country: "Fluent" },
  { name: "Hindi", level: 100, flag: "🇮🇳", country: "Fluent" },
];
    ...
    >
      <div className="sticky top-0 min-h-screen flex items-center overflow-hidden bg-muted/30 bg-mesh py-10">
        <div className="container mx-auto px-6">
          ...
          >
            <h2 className="text-4xl font-bold mb-4 mx-[100px] my-[50px] px-[100px] py-[10px] md:text-6xl">
              Skills &{" "}
...
function SkillCard({ category, isActive }: SkillCardProps) {
  return (
    <div
      className={`bg-card/90 backdrop-blur-sm border border-border rounded-2xl p-6 shadow-card transition-all duration-300 ${
        isActive ? "shadow-premium border-primary/30" : ""
      }`}
    >
